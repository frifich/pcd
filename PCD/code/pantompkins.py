...
def run(ecg_sequence, return_qrs_peaks_indices=False):
    try:
    ...
    PVC_flag = detect_pvc(qrs_peaks_indices)

    if return_qrs_peaks_indices:
        return qrs_peaks_indices, PVC_flag
    else:
        return PVC_flag
...
def detect_pvc(poses):

    pre = np.array([], dtype=int)
    current = np.array([], dtype=int)

    pre = np.append(pre, 0)
    current = np.append(current, poses[1] - poses[0])

    for i in range(1, len(poses)-1):
        pre = np.append(pre, poses[i] - poses[i-1])
        current = np.append(current, poses[i+1] - poses[i])
    try:
        pre[0] = pre[1]
    except:
        pass
    ...
    rr_avg = 0
    for interval in np.append(pre, current):
        rr_avg += interval
    rr_avg = rr_avg / (pre.size + current.size)
    upper = rr_avg * 1.01
    lower = rr_avg * 0.99
    count = 0
    for curr in current:
